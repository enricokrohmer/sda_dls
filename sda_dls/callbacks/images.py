...
class ImageLoggingCallback(Callback):
    ...
    def on_train_batch_end(self, trainer: Trainer, pl_module: LightningModule, *args, **kwargs) -> None:
        if self.deactivation_epoch is not None and trainer.current_epoch >= self.deactivation_epoch:
            return
...
class ImageStoringCallback(Callback):
    ...
    def on_predict_batch_end(self, trainer: Trainer, pl_module: LightningModule, outputs: STEP_OUTPUT, batch: Any, *args, **kwargs) -> None:
        images, paths = outputs
        for image, path in zip(images, paths):
            image = self._renormalize(image)
            class_name, name = self._get_class_and_name(path)
        
            target_dir = os.path.join(self.target_root, str(trainer.current_epoch), class_name)
