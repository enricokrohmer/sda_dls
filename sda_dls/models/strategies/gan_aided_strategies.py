...
class CycadaDANN(TwoStepStrategy):
    ...
    def _update_acc(self, preds, labels):
        ...
        if self.pl_module.trainer.world_size > 1:
            preds, labels = self.pl_module.all_gather(
                [preds, labels],
                sync_grads=True
            )
        ...
        self.pl_module.log('acc_disc', self.current_acc, on_step=True, on_epoch=False, sync_dist=True)
    ...
    def _forward_train(self, batch):
        ...
        lambda_ = self.lambda_disc if self.current_acc > self.acc_threshold else torch.tensor(0.0)
        ...
        self.domain = torch.cat([torch.zeros_like(pred_disc_A), torch.ones_like(pred_disc_B)], dim=0)
        
    def _eval_disc_loss(self):
        loss = self.criterion_disc(self.pred_disc, self.domain)
        self._update_acc(self.pred_disc, self.domain)
        
        self.pl_module.log('loss_disc', loss, on_step=True, on_epoch=False, sync_dist=True)
        return loss
