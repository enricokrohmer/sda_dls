...
class AbstractCycleGanStrategy(Strategy):
    ...
    def _eval_gan_loss(self):
        ...
        self.pl_module.log(
            "loss_gen_A", loss_A, on_step=True, on_epoch=False, sync_dist=True
        )
        self.pl_module.log(
            "loss_gen_B", loss_B, on_step=True, on_epoch=False, sync_dist=True
        )
    ...
    def _eval_cycle_loss(self):
        ...
        self.pl_module.log(
            "loss_cycle", loss_cycle, on_step=True, on_epoch=False, sync_dist=True
        )
        return loss_cycle
    ...
    def _eval_idt_loss(self):
        ...
        self.pl_module.log(
            "loss_idt", loss_idt, on_step=True, on_epoch=False, sync_dist=True
        )
        return loss_idt

    def _eval_fc_loss(self):
        loss_fc_A = self.criterion_fc(
            self.imgs["real_A"], self.imgs["fake_B"], self.imgs["real_B"]
        )
        loss_fc_B = self.criterion_fc(
            self.imgs["real_B"], self.imgs["fake_A"], self.imgs["real_A"]
        )
        loss_fc = loss_fc_A + loss_fc_B

        self.pl_module.log(
            "loss_fc", loss_fc, on_step=True, on_epoch=False, sync_dist=True
        )
        return loss_fc
...
class CycleGanStrategy(AbstractCycleGanStrategy):
    ...
    def backward_discriminators(self):
        ...
        self.pl_module.log(
            "loss_discB", loss_discB, on_step=True, on_epoch=False, sync_dist=True
        )
        loss_discA = self._bwd_single_disc(self.disc_A, self.imgs["real_A"], fake_A)
        self.pl_module.log(
            "loss_discA", loss_discA, on_step=True, on_epoch=False, sync_dist=True
        )
...
class CyCADAStrategy(CycleGanStrategy):
    ...
    def _eval_sem_loss(self):
        loss_sem = self.lambda_sem * self.criterion_sem(self.preds, self.labels)

        self.pl_module.log(
            "loss_sem", loss_sem, on_step=True, on_epoch=False, sync_dist=True
        )
        return loss_sem
