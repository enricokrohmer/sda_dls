...
class Strategy(ABC):
    ...
    def attach_pl_module(self, module: LightningModule):
        module_dict = {k: v for k, v in self.__dict__.items() 
                   if isinstance(v, nn.Module) and
                   not isinstance(v, LightningModule)}
    ...
    def compile(self):
        module_list = [v for v in self.__dict__.values() 
...
class AbstractClassifierStrategy(Strategy):
    ...
    def _eval_task_loss(self):
        ...
        self.pl_module.log('task_loss', task_loss, on_step=True, on_epoch=False, sync_dist=True)
...
class DANNStrategy(AbstractClassifierStrategy):
    ...
    def _forward_train(self, batch):
        ...
        self.domain = torch.cat((torch.zeros_like(disc_A), torch.ones_like(disc_B)), dim=0)
...
class DANStrategy(AbstractClassifierStrategy):
    ...
    def _eval_mmd_loss(self):
        if self.pl_module.trainer.world_size > 1:
            features_A, features_B, = self.pl_module.all_gather(
        ...
        loss = self.lambda_mmd * self.criterion_mmd(
            features_A, features_B
        )
        
        self.pl_module.log('mmd_loss', loss, on_step=True, on_epoch=False, sync_dist=True)
        return loss
